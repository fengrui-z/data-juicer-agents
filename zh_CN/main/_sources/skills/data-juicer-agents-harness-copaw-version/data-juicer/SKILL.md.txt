...
|---------|-------------|--------------------------|
| **dataset_path** | Input dataset path | Provided by user; AskUserQuestion if not specified |
...
djx tool run build_dataset_spec --input-json '{
  ...
  "dataset_path": "/data/articles.jsonl",
...
# Step 9: Verify
djx tool run inspect_dataset --input-json '{"dataset_path": "/data/processed/articles.jsonl", "sample_size": 10}'
```
...
|------|-----------------------|---------------|-------------|
| `build_dataset_spec` | dataset_path | `"dataset_path": "/path/to/data.jsonl"` | ~~`"input_path"`~~ |
...
```bash
# inspect_dataset
djx tool run inspect_dataset --input-json '{"dataset_path": "<INPUT>", "sample_size": 50}'
